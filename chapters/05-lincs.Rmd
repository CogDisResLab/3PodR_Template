...
lincs_output_1 <- global_state$data %>%
  purrr::map(~ knitr::knit_child(here::here("Rmd", "lincs_signatures.Rmd"), 
                                 envir = environment(), 
                                 quiet = TRUE)) %>%
  purrr::list_c()

knitr::asis_output(lincs_output_1)
}, error = function(e) {
...
tryCatch({
lincs_output_2 <- global_state$data %>%
  purrr::map(~ knitr::knit_child(here::here("Rmd", "lincs_metadata.Rmd"), 
                                 envir = environment(), 
                                 quiet = TRUE)) %>%
  purrr::list_c()

knitr::asis_output(lincs_output_2)
}, error = function(e) {
    # This code runs only if an error occurs
    message("An error occurred, returning NULL.")
    return(NULL)
...
lincs_output_4 <- list(LINCS_PAVER_result, "logCS") %>%
  knitr::knit_child(text = readLines(here::here("Rmd", "paver_report.Rmd")),
             envir = environment(),
             quiet = TRUE)

knitr::asis_output(lincs_output_4)
}, error = function(e) {
    # This code runs only if an error occurs
    message("An error occurred, returning NULL.")
    return(NULL)
